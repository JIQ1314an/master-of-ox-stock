...
parser.add_argument('--epochs', default=400, type=int)  # 训练轮数
parser.add_argument('--layers', default=2, type=int)  # LSTM层数
parser.add_argument('--input_size', default=14, type=int)  # 输入特征的维度dim
...
parser.add_argument('--better_loss', default=0.07, type=float)  # loss值
parser.add_argument("--seed", type=int, default=41, help="random seed.")  # 随机数种子

# ========================== Save the evaluation index Path ===========
parser.add_argument('--loss_writer', type=str, default=join(root_path, 'logs/loss'))
parser.add_argument('--r2_writer', type=str, default=join(root_path, 'logs/r2'))
parser.add_argument('--mse_writer', type=str, default=join(root_path, 'logs/mse'))
